...
const KhakKhordeIcon = () => (
    <svg width="36" height="36" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="1.2" fill="white"/>
...
const cardConfigs = {
  active:   { color: "#38A3A5", icon: <RumizIcon />,         labelKey: "taskPreviewCard.active",   gradient: "linear-gradient(135deg, #38A3A5 0%, #4AB8BB 100%)", endpoint: "rumiz" },
  upNext:   { color: "#57CC99", icon: <NobateshMisheIcon />, labelKey: "taskPreviewCard.upNext",   gradient: "linear-gradient(135deg, #57CC99 0%, #6DE2AF 100%)", endpoint: "nobatesh_mishe" },
  archived: { color: "#80ED99", icon: <KhakKhordeIcon />,    labelKey: "taskPreviewCard.archived", gradient: "linear-gradient(135deg, #80ED99 0%, #96F5AF 100%)", endpoint: "khak_khorde" },
};
...
const fetchByEndpoint = async (endpoint) => {
  let data;
  switch (endpoint) {
    case 'khak_khorde':    data = await tasksAPI.getKhakKhordeTasks(); break;
    case 'rumiz':          data = await tasksAPI.getRumizTasks(); break;
    case 'nobatesh_mishe': data = await tasksAPI.getNobateshMisheTasks(); break;
    default: return [];
  }

  if (Array.isArray(data)) return data;
  if (data && typeof data === 'object') {
    if (endpoint === 'rumiz') return [...(data.completed_tasks || []), ...(data.not_completed_tasks || [])];
    return data.tasks || data.not_completed_tasks || data.completed_tasks || [];
  }
  return [];
};
...
function TaskPreviewCard({ cardName, setSelectedTask, selectedTask }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    fetchByEndpoint(config.endpoint).then(raw => {
      if (!cancelled) setTasks(raw.map((t, i) => normalizeTask(t, i)));
    })
  ...
  const handleTaskClick = (task) => {
    if (!task?.id) return;
    if (!timers[task.id]) setTimers(prev => ({ ...prev, [task.id]: { elapsed: 0, isRunning: false } }));
  ...
  const handlePlayPause = (e, task) => {
    ...
    timer.isRunning ? stopTimer(task.id) : startTimer(task.id);
  };
  ...
  return (
      <div className="task-preview__card" style={{ background: config.gradient }}>
        ...
        <div className="task-preview__card-tasks">
          {loading ? (
              <div className="empty-state"><span>در حال بارگذاری...</span></div>
          ) : tasks.length === 0 ? (
              <div className="empty-state"><span>{t("taskPreviewCard.noTasks")}</span></div>
          ) : (
              tasks.map((task) => {
                    ...
                    >
                      <div className="task-content">
                        ...
                        <div className="task-info">
                          <span className="task-title" title={task.title}>{task.title}</span>{totalTime > 0 && <span className="task-time">{formatTime(totalTime)}</span>}
                        </div>
                      </div>
                      <button
