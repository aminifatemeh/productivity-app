...
function SubtaskBar({ className = '', onDelete, isDone, onToggle, title }) {
    ...
    return (
      <div
        className={`subtaskbar ${className} ${isDone ? "done" : "not-done"}`}
      >
        <span>{title}</span>
        <div className="d-flex justify-content-center align-items-center gap-1">
          ...
          />
          <img src={isDone ? "/assets/icons/green-tick.svg" : "/assets/icons/gray-tick.svg"} alt="Done" onClick={handleToggleDone} />
        </div>
      </div>
    );
}
